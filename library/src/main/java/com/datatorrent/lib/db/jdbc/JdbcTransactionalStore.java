      ...
      if (!rsColumns.isBeforeFirst()) {
        ...
        if (!rsColumns.isBeforeFirst()) {
          ...
          if (!rsColumns.isBeforeFirst()) {
            ...
            String createMetaTable = create.createTable(JdbcTransactionalStore.DEFAULT_META_TABLE)
                .column(JdbcTransactionalStore.DEFAULT_APP_ID_COL, SQLDataType.VARCHAR.length(100).nullable(false))
                .column(JdbcTransactionalStore.DEFAULT_OPERATOR_ID_COL, SQLDataType.INTEGER.nullable(false))
                .column(JdbcTransactionalStore.DEFAULT_WINDOW_COL, SQLDataType.BIGINT.nullable(false))
                .getSQL(ParamType.INLINED);
            createMetaTable = createMetaTable.replace('"',' ');
            connection.prepareStatement(createMetaTable).executeUpdate();
            createMetaTable = create.alterTable(JdbcTransactionalStore.DEFAULT_META_TABLE)
              .add(constraint("Unique").unique(JdbcTransactionalStore.DEFAULT_APP_ID_COL,
                JdbcTransactionalStore.DEFAULT_OPERATOR_ID_COL,
                JdbcTransactionalStore.DEFAULT_WINDOW_COL)).getSQL(ParamType.INLINED);
            createMetaTable = createMetaTable.replace('"',' ');
            connection.prepareStatement(createMetaTable).executeUpdate();
          }
        }
